...
class CommentHttp extends Http {
  ...
  deleteComment = async (commentId, userId) => {
    try {
      const res = await this.axios.delete(
        `board/comment?comment_id=${commentId}&user_id=${userId}`
      );
      return res;
    } catch (err) {
      throw err;
  ...
  deleteReply = async (replyId, userId) => {
    try {
      const res = await this.axios.delete(
        `board/comment/reply?reply_id=${replyId}&user_id=${userId}`
      );
      return res;
    } catch (err) {
      throw err;
  ...
  //유저 별 글 목록 조회
  getWrittenList = async (userId, num) => {
    try {
      const res = await this.axios.get(`board/${userId}/list?page=${num}`);
      return res;
    } catch (err) {
      throw err;
    }
