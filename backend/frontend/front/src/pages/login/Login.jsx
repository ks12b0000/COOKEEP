...
function Login() {
    //카카오 로그인 요청 주소
    const KakaoURL = `https://kauth.kakao.com/oauth/authorize?client_id=${process.env.REACT_APP_KAKAO_REST_KEY}&redirect_uri=https://www.teamprojectvv.shop/callback/kakao&response_type=code
    `;

    //구글 로그인 요청 주소
    const googleURL = `https://accounts.google.com/o/oauth2/v2/auth/oauthchooseaccount?client_id=${process.env.REACT_APP_GOOGLE_REST_KEY}&redirect_uri=https://www.teamprojectvv.shop/callback/google&response_type=code&scope=email`;

    //네이버 로그인 요청 주소
    const state = Math.floor(new Date().getTime() + Math.random() * 1000);
    const NaverURL = `https://nid.naver.com/oauth2.0/authorize?client_id=${process.env.REACT_APP_NAVER_REST_KEY}&response_type=code&redirect_uri=https://www.teamprojectvv.shop/callback/naver&state=${state}`;
